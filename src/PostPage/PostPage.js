...
function PostPage() {
  ...
  const onClickLike = () => {
    postData.isLiked
      ? setPostData({ ...postData, isLiked: false })
      : setPostData({ ...postData, isLiked: true });
  };

  const onClickStore = () => {
    postData.isStored
      ? setPostData({ ...postData, isStored: false })
      : setPostData({ ...postData, isStored: true });
  };
...
function PostCafeInfo({
  ...
  const clickLikeBtn = useCallback(() => {
    console.log('좋아요클릭');
    ...
    axios
      .get(`http://api.cpp.co.kr:3300/posts/${id}/like`, {
  ...
  const clickStoreBtn = useCallback(() => {
    console.log('저장하기클릭');
    if (!sessionStorage.getItem('user')) {
      alert('로그인된 사용자가 아닙니다');
      return;
    ...
    axios
      .get(`http://api.cpp.co.kr:3300/posts/${id}/store`, {
        withCredentials: true,
      })
      .then((res) => {
        onClickStore();
